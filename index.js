  ...
  axios
    ...
    .then(async ({ data: alles }) => {
      ...
      if (
        ...
        return res
          .status(400)
          .send("This discord account or AllesID is already connected!");
...
// Commands
const help = fs
  .readFileSync(`${__dirname}/help.md`, "utf8")
  .replace(/\$/g, process.env.PREFIX);
const commands = {
  ...
  link: async (msg) => {
    try {
      ...
      await msg.author.send(
        `Hey! Ready to link your AllesID? Use this link: ${process.env.ORIGIN}/token/${token}`
      );
      await msg.channel.send(
        "Awesome! I've sent you a dm, just click the link to connect your AllesID."
      );
    } catch (err) {
      await msg.channel.send(
        `Sorry, ${msg.author}, I tried to dm you but something went wrong. Make sure that you are allowing dms from me :)`
      );
    }
  },
  me: async (msg) => {
  ...
  xp: async (msg) => {
    ...
    if (user.xp.level >= 50)
      return await msg.channel.send(
        `bruh, you're on level ${user.xp.level} smh`
      );

    // Check cooldown
    if (
      xpDates[user.id] &&
      xpDates[user.id] > new Date().getTime() - 1000 * 60 * 60
    ) {
      const minsLeft = Math.ceil(
        (xpDates[user.id] + 1000 * 60 * 60 - new Date().getTime()) / (1000 * 60)
      );
      return await msg.channel.send(
        `Hold up, ${msg.author}! You still have to wait ${minsLeft} minute${
          minsLeft === 1 ? "" : "s"
...
// Escape
const esc = (content) =>
  content.replace(/@everyone/g, "@.everyone").replace(/@here/g, "@.here");

// Nexus
const nexus = async (method, endpoint, data) =>
  (
    await axios({
      method,
...
const userStats = async (id, msg) => {
  ...
  await msg.channel.send(
    `**ID:** ${user.id}\n` +
      ...
      `${"▓".repeat(progress)}${"░".repeat(10 - progress)} ${Math.floor(
