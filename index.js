    ...
    (await server.members.fetch()).forEach(async (member) => {
      ...
      try {
        ...
        if (
          ...
          (member.user.username !== nickname || !member.user.username)
        )
      ...
      try {
        ...
        if (user && !roles.includes(process.env.LINKED_ROLE))
          await member.roles.add(linkedRole);
        if (!user && roles.includes(process.env.LINKED_ROLE))
          await member.roles.remove(linkedRole);
        if (user && user.plus && !roles.includes(process.env.PLUS_ROLE))
          await member.roles.add(plusRole);
        if (!user || (!user.plus && roles.includes(process.env.PLUS_ROLE)))
          await member.roles.remove(plusRole);
      } catch (err) {}
    });
...
const getAllesId = async (discord) =>
  (
    await axios.get(
      `https://discord.alles.cc/discord/${encodeURIComponent(discord)}`
    )
  ).data.alles;
