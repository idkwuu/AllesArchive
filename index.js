...
app.get("/nametag", (req, res) => {
    ...
    nexus("GET", `${process.env.NEXUS_URI}/nametag?name=${encodeURIComponent(req.query.name)}&tag=${encodeURIComponent(req.query.id)}`)
        .then(({id}) => res.json({id}))
...
app.get("/users/:id", (req, res) => {
    nexus("GET", `${process.env.NEXUS_URI}/users/${encodeURIComponent(req.params.id)}`)
        .then(user => res.json({
            id: user.id,
            name: user.name,
            tag: user.tag,
